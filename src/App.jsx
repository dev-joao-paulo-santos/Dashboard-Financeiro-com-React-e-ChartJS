...
function App() {
  ...
  const checkRules = () => {
    ...
    if(spendingPercent >= 60){
      setWarningMessage("Você ultrapassou o limite de 60% do seu saldo inicial! Reveja suas despesas!")
      setWarningClass('text-red-800, bg-red-400')
    } else if (spendingPercent >= 50) {
      setWarningMessage("Cuidado! Você está gastando mais da metade do seu saldo inicial!")
  ...
  const exportToExcel = (expenses, initialBalance) => {
    ...
    const wsData = [
      [{ v: "Saldo inicial", s: { font: { bold: true }, alignment: { horizontal: "center" } } }, initialBalance],
      [],
  ...
  return (
    <div className="min-h-screen overflow-y-hidden bg-gradient-to-r from-emerald-950 font-[Poppins] to-cyan-700 flex flex-col items-center">
     <div data-aos="fade-down" className="bg-white/20 h-screen mx-auto mb-7 pt-4 w-full md:w-3/4 flex flex-col justify-center">
      <h1
      ...
      <div className="my-6 flex flex-col items-center">
        <label className="block font-bold mb-2">Insira seu saldo inicial</label>
        <input
          type="number"
        ...
        />
        <button onClick={updateBalance} className="duration-300 bg-green-600 w-1/6 text-white px-4 py-2 rounded-full my-4 hover:bg-green-800">
          Atualizar Saldo
        </button>
      ...
      <button onClick={ () => exportToExcel(expenses, initialBalance)} className="px-4 py-3 text-xl bg-lime-700 rounded-full text-white my-14">Exportar para Excel</button>
    </div>
  );
}
